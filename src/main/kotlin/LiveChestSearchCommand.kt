...
class LiveChestSearchCommand(private val hound: Hound): TabExecutor {
    ...
    override fun onCommand(sender: CommandSender, command: Command, label: String, args: Array<out String>): Boolean {
        ...
        if (!sender.hasPermission("hound.search.containers.live")) {
        ...
        if (liveSearchMode) {
            if (args.isNotEmpty() && args[0].toLowerCase() == "on") {
                sender.sendMessage("§4Live search mode is already active.")
                return true
            }
        ...
        if (args.isNotEmpty() && args[0].toLowerCase() == "off") {
            sender.sendMessage("§4Live search mode is already inactive.")
            return true
        }
